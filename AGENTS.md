...
| **`dungeon.js`** (~364) | **Procedural room-graph dungeon** per depth: difficulty/reward scaling, room geometry/collision/door transitions, enemy spawning, biome rendering. At **`FINAL_DEPTH`** the boss room spawns the campaign's final boss (The Heart of Winter, `isFinal`) instead of the biome boss — felling it is the win. | `Dungeon`, `dungeonConfig(depth)`, `biomeForDepth(depth)`, `depthColor(depth)`, `FINAL_DEPTH` |
