...
| **`hud.js`** (~517) | **HUD + overlay rendering.** Every fn takes the per-frame `view` bundle from main.js `render()` and destructures what it needs. World-space draws (projectiles/pickups/portals/ambient/spawners/chain) run inside the camera transform; the rest are screen-space. `drawItemPickups` renders the **gear-acquired cards** (the `view.itemPickups` feed, pushed by main.js on item pickup) — a rich icon+type+name+⚡power card per drop, visually distinct from the coin `drawToasts`; it reuses `drawItemIcon`/`slotTint`/`slotTag`/`withAlpha`/`clipText` re-exported from `inventory.js`. | `drawHud`, `drawBanner`, `drawToasts`, `drawItemPickups`, `drawCombo`, `drawBossBar`, `drawRoomMap`, `drawGameOver`, `drawDamageIndicator`, `drawLowHpVignette`, `drawPrompt(view,text,color)`, `drawEntranceTooltip(view,dg)`, `drawProjectiles`/`drawPickups`/`drawPortals`/`drawAmbient`/`drawSpawners`/`drawChainTarget` |
